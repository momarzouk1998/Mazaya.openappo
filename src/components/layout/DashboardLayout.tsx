...
export default function DashboardLayout({ children }: Props) {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      {/* Sidebar */}
      <aside className={`fixed lg:sticky top-0 start-0 h-dvh w-64 bg-brand-black text-white z-40 transition-transform flex flex-col ${sidebarOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0"}`}>
        ...
        <div
          className="shrink-0 p-3 border-t border-white/10 bg-brand-black"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* pb الإضافي على الموبايل عشان محتوى الصفحة ما يتغطّاش بشريط التنقّل السفلي */}
        <main className="flex-1 px-4 pt-4 md:px-6 md:pt-6 pb-[calc(4.5rem+env(safe-area-inset-bottom))] lg:pb-6 overflow-x-hidden">
          {children}
        </main>
      </div>
