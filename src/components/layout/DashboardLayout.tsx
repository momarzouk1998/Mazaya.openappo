...
export default function DashboardLayout({ profile, children }: Props) {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      {/* Sidebar */}
      <aside className={`fixed lg:sticky top-0 start-0 h-screen w-64 bg-brand-black text-white z-40 transition-transform flex flex-col print:hidden ${sidebarOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0"}`}>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 bg-black/50 z-30 lg:hidden print:hidden" onClick={() => setSidebarOpen(false)} />
      )}

      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-gray-200 sticky top-0 z-20 px-4 py-3 flex items-center justify-between print:hidden">
