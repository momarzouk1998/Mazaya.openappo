...
export default function BoardsWalletPage() {
  ...
  const totals = data?.totals ?? { total_income: 0, total_expense: 0, current_balance: 0 };
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div className="card bg-white border-r-4 border-green-400">
            ...
            <div className="text-2xl font-extrabold mb-1 text-green-600">+{formatCurrency(totals.total_income)}</div>
            <div className="text-xs text-green-500/70">كل التحويلات التمريرية</div>
          </div>
          <div className="card bg-white border-r-4 border-red-400">
            <div className="text-xs text-gray-500 font-bold">إجمالي المشتريات</div>
            <div className="text-2xl font-extrabold mb-1 text-red-600">-{formatCurrency(totals.total_expense)}</div>
            <div className="text-xs text-red-400">كل مشتريات الألواح</div>
          </div>
          <div className={`card bg-gradient-to-br ${totals.current_balance < 0 ? "from-red-500 to-red-700" : "from-brand-orange to-brand-orange-dark"} text-white col-span-2 md:col-span-1`}>
            ...
            <div className="text-xs opacity-80">وارد − مشتريات (كامل)</div>
          </div>
        </div>
      </div>
