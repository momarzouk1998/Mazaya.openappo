...
export default function BoardsDayReportPage() {
  ...
  const query = `/api/boards-wallet?date_from=${dateStr}&date_to=${dateStr}`;
  ...
  function handleExportExcel() {
    const clean = filteredEntries.map((e, idx) => {
      ...
      return {
        "#": idx + 1,
        التاريخ: dateStr,
        "نوع الحركة": isIncome ? "وارد (تمريري)" : (ENTRY_TYPE_LABELS[e.entry_type] || e.entry_type),
        البيان: cleanDescription(e.description),
        "المورد / الجهة": e.party_name || "—",
        "طريقة الدفع": PAYMENT_METHOD_LABELS[e.payment_method || "نقدي"] || e.payment_method || "نقدي",
        الوارد: isIncome ? fmtNum(e.amount) : 0,
        المشتريات: !isIncome ? fmtNum(e.amount) : 0,
        الملاحظات: e.notes || "—",
      };
    });

    exportToExcel(clean, `تقرير_يومية_الألواح_${dateStr}`);
  }
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await downloadElementAsPdf({
        elementId: "printable-boards-day-report",
        fileName: `تقرير_يومية_الألواح_${dateStr}`,
    ...
    <DashboardLayout profile={profile}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div>
          <h1 className="text-base font-extrabold text-gray-900 flex items-center gap-2">
            <span>🪵</span>
            <span>تقرير اليوم — {dayName} {formatDate(dateStr)} (يومية الألواح)</span>
          </h1>
        </div>
        ...
        <table className="w-full text-center text-xs">
          <thead className="bg-gray-100/90 text-gray-700 border-b border-gray-200 font-bold">
            <tr>
              ...
              <th className="p-2.5 text-center text-emerald-800">📥 إجمالي الوارد (التمريري)</th>
              <th className="p-2.5 text-center text-rose-800">📤 إجمالي المشتريات</th>
      ...
      <div className="bg-white p-2 rounded-xl border border-gray-200 shadow-xs mb-2.5 flex items-center justify-between gap-2 no-print">
        <span className="text-xs font-bold text-gray-700">
          حركات الألواح المسجلة ({filteredEntries.length})
        </span>

        <div className="relative w-56 sm:w-72">
          <input
            type="text"
            placeholder="🔍 بحث في البيان أو المورد أو المبلغ..."
            value={search}
      ...
      {loading ? (
        <div className="card text-center py-12 bg-white border no-print">
          ...
          <div className="text-xs font-bold text-gray-600">جاري تحميل بيانات يومية الألواح...</div>
        </div>
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-100/80 text-gray-800 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-2.5 py-2 text-center font-extrabold whitespace-nowrap">المورد / الجهة</th>
                  ...
                  <th className="px-2.5 py-2 text-center font-extrabold whitespace-nowrap text-rose-800">المشتريات (-)</th>
                </tr>
              </thead>
                        ...
                        <span
                          className={`inline-block px-2 py-0.5 rounded text-[11px] font-bold ${
                            isIncome
                              ...
                              : ENTRY_TYPE_COLORS[e.entry_type] || "bg-gray-100 text-gray-800"
                          }`}
                        >
              ...
              <tfoot className="bg-gray-50 border-t-2 border-gray-300 font-extrabold text-xs text-gray-800">
                <tr>
                  ...
                  <td colSpan={4} className="px-3 py-2 text-start">
                    إجمالي حركة اليوم ({filteredEntries.length} حركة)
                  </td>
                  <td className="px-2.5 py-2 text-center font-mono font-bold text-emerald-800 whitespace-nowrap">
                    +{formatCurrency(filteredEntries.filter((e) => e.entry_type === "دفعة صادرة لمورد" && e.is_pass_through).reduce((s, e) => s + fmtNum(e.amount), 0))}
                  </td>
        ...
        <div className="card text-center text-gray-400 py-12 bg-white border rounded-xl no-print">
          ...
          <div className="font-bold text-gray-700 text-sm">لا توجد حركات ألواح مسجلة في هذا اليوم</div>
          <div className="text-xs text-gray-400 mt-0.5">لم تسجل أي حركة مشتريات أو تحويلات تمريرية في هذا التاريخ.</div>
        </div>
      )}
      ...
      {/* ======================================================== */}
      <div
        id="printable-boards-day-report"
        style={{
          position: "fixed",
          left: "-9999px",
          top: "0",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "2px solid #ea580c", paddingBottom: "10px", marginBottom: "14px" }}>
          <div>
            ...
            <h2 style={{ fontSize: "13px", fontWeight: "700", color: "#ea580c", marginTop: "3px", margin: 0 }}>
              تقرير اليوم — {dayName} {formatDate(dateStr)} (يومية الألواح)
            </h2>
          </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px", textAlign: "center", marginBottom: "14px" }}>
          <thead>
            <tr style={{ backgroundColor: "#f3f4f6", borderBottom: "2px solid #d1d5db" }}>
              ...
              <th style={{ padding: "6px", border: "1px solid #e5e7eb", color: "#166534" }}>📥 إجمالي الوارد (التمريري)</th>
              <th style={{ padding: "6px", border: "1px solid #e5e7eb", color: "#9f1239" }}>📤 إجمالي المشتريات</th>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "9.5px", textAlign: "center" }}>
          <thead>
            <tr style={{ backgroundColor: "#f3f4f6", borderBottom: "2px solid #d1d5db" }}>
              ...
              <th style={{ padding: "5px 4px", border: "1px solid #e5e7eb", fontWeight: "800", whiteSpace: "nowrap" }}>المورد / الجهة</th>
              ...
              <th style={{ padding: "5px 4px", border: "1px solid #e5e7eb", fontWeight: "800", whiteSpace: "nowrap" }}>المشتريات (-)</th>
            </tr>
          </thead>
          <tbody>
            {dayData.entries.map((e, i) => {
              ...
              return (
                <tr key={i} style={{ backgroundColor: i % 2 === 0 ? "#ffffff" : "#f9fafb" }}>
                  ...
                  <td style={{ padding: "4px 3px", border: "1px solid #e5e7eb", whiteSpace: "nowrap", fontWeight: "700" }}>{isIncome ? "وارد (تمريري)" : (ENTRY_TYPE_LABELS[e.entry_type] || e.entry_type)}</td>
          ...
          <tfoot>
            <tr style={{ backgroundColor: "#fef3c7", borderTop: "2px solid #d97706", fontWeight: "900", fontSize: "10.5px" }}>
              ...
              <td style={{ padding: "5px 3px", border: "1px solid #fde68a", fontFamily: "monospace", color: "#15803d" }}>+{formatCurrency(filteredEntries.filter((e) => e.entry_type === "دفعة صادرة لمورد" && e.is_pass_through).reduce((s, e) => s + fmtNum(e.amount), 0))}</td>
              <td style={{ padding: "5px 3px", border: "1px solid #fde68a", fontFamily: "monospace", color: "#be123c" }}>-{formatCurrency(filteredEntries.filter((e) => !(e.entry_type === "دفعة صادرة لمورد" && e.is_pass_through)).reduce((s, e) => s + fmtNum(e.amount), 0))}</td>
            </tr>
          </tfoot>
        </table>
      </div>

      {/* تنسيقات الطباعة الطولية المتصلة السلسة */}
      <style jsx global>{`
        @media print {
          @page {
            size: A4 portrait;
