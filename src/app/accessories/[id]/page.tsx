...
import type { AccessoriesInventory, Supplier, OrderMaterial, Order } from '@/types';
...
export default function AccessoryDetailPage() {
  ...
  const [item, setItem] = useState<AccessoryWithSupplier | null>(null);
      ...
      try {
        const [iRes, uRes] = await Promise.all([
          ...
          supabase
            ...
            .select('*')
            .eq('item_category', 'accessory')
            .eq('item_id', id)
            .order('created_at', { ascending: false }),
        ]);

        setItem(iRes.data as AccessoryWithSupplier);
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-12">
          ...
          <Link href="/accessories" className="text-orange-600 hover:text-orange-700 mt-2 inline-block">
            ← العودة لمخزون الاكسسوارات
          </Link>
        </div>
      </DashboardLayout>
    );
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-4xl">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          <InfoItem label="النوع" value={item.accessory_type} />
