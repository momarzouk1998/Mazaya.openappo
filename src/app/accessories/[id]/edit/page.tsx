...
export default function EditAccessoryPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [itemRes, supRes, typeRes, ordRes] = await Promise.all([
          supabase.from('accessories_inventory').select('*').eq('id', id).single(),
          supabase.from('suppliers').select('*').order('name'),
          supabase.from('accessory_types').select('*').order('name'),
          supabase.from('orders').select('*').order('created_at', { ascending: false }),
        ]);

        const item = itemRes.data as AccessoriesInventory;
        if (item) {
          setForm({
  ...
  return (
    <DashboardLayout>
      <div className="max-w-xl space-y-6">
        <div>
          <Link href={`/accessories/${id}`} className="text-sm text-orange-600 hover:text-orange-700 mb-1 inline-block">
            ← الصنف
          </Link>
