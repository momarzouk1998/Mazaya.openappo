...
import type { Supplier, MaterialType, Order } from '@/types';
...
export default function NewBoardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [supRes, matRes, ordRes] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('boards_inventory').insert({
