...
import { INCOME_TYPES, FACTORY_EXPENSE_TYPES, PAYOUT_TYPES } from '@/lib/finance';
  ...
  try {
    ...
    for (const date of sortedDates) {
      ...
      const expense = dayRows
        .filter(r => (FACTORY_EXPENSE_TYPES as readonly string[]).includes(r.entry_type) && !r.is_pass_through)
        .reduce((s, r) => s + toNum(r.amount), 0);
