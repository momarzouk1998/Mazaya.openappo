...
export async function DELETE(
  ...
  try {
    ...
    // id can be overhead_expenses id or journal_entries id
    ...
    if (!overhead) {
      return NextResponse.json(
        { ok: false, error: { code: 'NOT_FOUND', message: 'سجل مصاريف الدهانات غير موجود' } },
        { status: 404 }
      );
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 2) حذف قيد اليومية المرتبط إن وجد
      if (journalEntryId) {
    ...
    console.error('Paints DELETE error:', e);
